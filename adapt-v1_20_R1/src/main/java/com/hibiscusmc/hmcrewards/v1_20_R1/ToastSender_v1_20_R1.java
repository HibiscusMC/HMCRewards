package com.hibiscusmc.hmcrewards.v1_20_R1;
...
public final class ToastSender_v1_20_R1 implements ToastSender {
    ...
    public void showToast(final @NotNull Player player, final @NotNull ItemStack icon, final @NotNull Component title) {
        ...
        final var advancement = Advancement.Builder.fromJson(json, new DeserializationContext(key, MinecraftServer.getServer().getLootData())).build(key);

        final var nmsPlayer = ((CraftPlayer) player).getHandle();
        final var progress = nmsPlayer.getAdvancements().getOrStartProgress(advancement);
        progress.getRemainingCriteria().forEach(criteria -> nmsPlayer.getAdvancements().award(advancement, criteria));

        Bukkit.getScheduler().runTaskLater(plugin, () -> {
            progress.getRemainingCriteria().forEach(criteria -> nmsPlayer.getAdvancements().revoke(advancement, criteria));
        }, 2L);
    }
}
